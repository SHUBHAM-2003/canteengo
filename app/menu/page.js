...
export default function MenuPage() {
  ...
  const categories = ['All', 'Meals', 'Snacks', 'Drinks', 'Beverages']
  const filtered = category === 'All' ? menuItems : menuItems.filter(i => i.name && menuItems.find(m => m.id === i.id)?.categories?.name === category)
  // Since Supabase join returns categories.name, we filter by category via category_id
  const catMap = { 'All': 'All', 'Meals': 'Meals', 'Snacks': 'Snacks', 'Drinks': 'Drinks', 'Beverages': 'Beverages' }
  ...
  const placeOrder = async () => {
    ...
    const items = cartItems.map(i => ({ item_id: i.id, name: i.name, price: i.price, quantity: i.qty }))
    try {
      const r = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          items, total_amount: cartTotal, payment_mode: paymentMode
        })
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f9fa' }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 64, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => setShowCart(true)} style={{ background: '#f8f9fa', padding: 10, borderRadius: 12, border: 'none', cursor: 'pointer', position: 'relative' }}>
            🛒
      ...
      <main style={{ marginTop: 64, padding: 20, paddingBottom: 100, maxWidth: 1200, marginLeft: 'auto', marginRight: 'auto' }}>
        {banners.length > 0 && (
          <div style={{ marginBottom: 20, borderRadius: 16, overflow: 'hidden', height: 180, backgroundImage: `url(${banners[currentBanner]?.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
            <div style={{ background: 'linear-gradient(transparent, rgba(0,0,0,0.7))', width: '100%', height: '100%', display: 'flex', alignItems: 'flex-end', padding: 16, color: 'white' }}>
              <div>
                <h3 style={{ margin: 0 }}>{banners[currentBanner]?.title}</h3>
                <p style={{ margin: '4px 0 0', fontSize: 14 }}>{banners[currentBanner]?.description}</p>
              </div>
            </div>
          </div>
        )}

        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '8px 0 20px', marginBottom: 8 }}>
          {categories.map(c => (
            <button key={c} onClick={() => setCategory(c)} style={{ padding: '12px 24px', background: category === c ? '#FF6B35' : 'white', color: category === c ? 'white' : '#636E72', borderRadius: 24, fontWeight: 600, border: '2px solid transparent', whiteSpace: 'nowrap', cursor: 'pointer' }}>
              {c}
            </button>
          ))}
        </div>
      ...
      {cartItems.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'white', padding: '14px 20px', boxShadow: '0 -4px 20px rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 100 }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '100%', maxWidth: 400, background: 'white', zIndex: 201, display: 'flex', flexDirection: 'column' }}>
                ...
                <div key={item.id} style={{ display: 'flex', gap: 14, padding: '14px 0', borderBottom: '1px solid #e8e8e8' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
                      ...
                      <button onClick={() => updateQty(item.id, -item.qty)} style={{ background: 'none', color: '#D63031', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Remove</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{ padding: 20, borderTop: '1px solid #e8e8e8', background: '#f8f9fa' }}>
              ...
              <button onClick={() => { setShowCart(false); setShowCheckout(true) }} disabled={cartItems.length === 0} style={{ width: '100%', background: '#FF6B35', color: 'white', padding: 16, borderRadius: 12, fontWeight: 600, border: 'none', cursor: 'pointer', opacity: cartItems.length === 0 ? 0.5 : 1 }}>Place Order</button>
            </div>
          </div>
        </>
      )}
          ...
          <div style={{ background: 'white', borderRadius: 24, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 24 }}>
              <div style={{ background: '#f8f9fa', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                {cartItems.map(item => (
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #e8e8e8' }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                <button onClick={() => { setPaymentMode('counter'); setShowPayment(false) }} style={{ flex: 1, padding: 16, border: `2px solid ${paymentMode === 'counter' ? '#FF6B35' : '#e8e8e8'}`, borderRadius: 12, background: paymentMode === 'counter' ? 'rgba(255,107,53,0.08)' : 'white', cursor: 'pointer', textAlign: 'center' }}>
                  ...
                  <h4 style={{ margin: '8px 0 4px' }}>Pay at Counter</h4>
                ...
                <button onClick={() => setPaymentMode('online')} style={{ flex: 1, padding: 16, border: `2px solid ${paymentMode === 'online' ? '#FF6B35' : '#e8e8e8'}`, borderRadius: 12, background: paymentMode === 'online' ? 'rgba(255,107,53,0.08)' : 'white', cursor: 'pointer', textAlign: 'center' }}>
                  <div style={{ fontSize: '1.5rem' }}>💳</div>
                  <h4 style={{ margin: '8px 0 4px' }}>Pay Online</h4>
            ...
            <div style={{ padding: '20px 24px', borderTop: '1px solid #e8e8e8', display: 'flex', gap: 12 }}>
              ...
              <button onClick={placeOrder} disabled={!paymentMode || (paymentMode === 'online' && !showPayment)} style={{ flex: 1, padding: 14, borderRadius: 12, fontWeight: 600, border: 'none', background: !paymentMode || (paymentMode === 'online' && !showPayment) ? '#B2BEC3' : '#FF6B35', color: 'white', cursor: !paymentMode || (paymentMode === 'online' && !showPayment) ? 'not-allowed' : 'pointer' }}>Confirm Order</button>
            </div>
          </div>
        </div>
      )}
