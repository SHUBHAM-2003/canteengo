...
export default function HomePage() {
  ...
  useEffect(() => {
    fetchJSON('/api/banners').then(data => setBanners((data || []).filter(b => b.is_active))).catch(() => {})
    const lastShown = typeof window !== 'undefined' && localStorage.getItem('moodShown')
  ...
  const handleMood = (msg) => {
    setShowMood(false)
    if (typeof window !== 'undefined') localStorage.setItem('moodShown', new Date().toDateString())
    setMoodToast(msg)
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 40px', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: '2rem' }}>🍽️</span>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FF6B35', margin: 0 }}>CanteenGo</h1>
        </div>
      ...
      <section style={{ textAlign: 'center', padding: '80px 20px 60px', background: 'linear-gradient(180deg, #FF6B35 0%, #E55A2B 100%)', color: 'white' }}>
        <h1 style={{ fontSize: '3rem', fontWeight: 700, margin: '0 0 16px' }}>Delicious Food,<br/>Just a Tap Away</h1>
        <p style={{ fontSize: '1.2rem', opacity: 0.9, margin: '0 0 32px' }}>Order from your table, track live, and enjoy quick pickup</p>
        <a href="/menu"><button style={{ background: 'white', color: '#FF6B35', padding: '16px 48px', borderRadius: 24, fontSize: '1.1rem', fontWeight: 700, border: 'none', cursor: 'pointer', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>Order Now</button></a>
      </section>

      <section style={{ display: 'flex', justifyContent: 'center', gap: 32, padding: '60px 40px', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={i} style={{ background: 'white', padding: 32, borderRadius: 16, textAlign: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', width: 280 }}>
      ...
      {banners.length > 0 && (
        <section style={{ padding: '0 40px 40px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: 12 }}>
            {banners.map((_, i) => <span key={i} onClick={() => setCurrentBanner(i)} style={{ width: 10, height: 10, borderRadius: '50%', background: i === currentBanner ? '#FF6B35' : '#B2BEC3', cursor: 'pointer' }} />)}
          </div>
        </section>
      )}

      <footer style={{ textAlign: 'center', padding: 40, background: '#2D3436', color: 'white' }}>
        <h3 style={{ fontSize: '1.5rem', margin: '0 0 8px' }}>CanteenGo</h3>
        <p style={{ margin: 0, opacity: 0.7 }}>Your campus food solution</p>
      </footer>

      {showMood && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowMood(false)}>
          <div style={{ background: 'white', borderRadius: 24, padding: 40, maxWidth: 400, width: '90%', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
